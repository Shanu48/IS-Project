...
(async function globalFaviconWhitelistSetup() {
  ...
  async function seedWhitelist(domainItems = []) {
    ...
    console.log("Starting favicon whitelist seeding for", domainItems.length, "domains...");
    ...
    for (const item of domainItems) {
      ...
      try {
        ...
        try {
          ...
          console.warn(`Tab for ${domain} did not finish loading in time:`, err);
        ...
        if (!payload) {
          ...
          console.log(`Domain ${domain} => favicon: ${payload.faviconUrl}, sha256: ${payload.sha256}`);
    ...
    const existing = await new Promise((resolve) => chrome.storage.local.get(['faviconHashWhitelist'], resolve));
    const existingMap = (existing && existing.faviconHashWhitelist) || {};

    const merged = Object.assign({}, existingMap, results);

    // Save to chrome.storage.local
    await new Promise((resolve) => chrome.storage.local.set({ faviconHashWhitelist: merged }, resolve));

    console.log("Favicon whitelist seeding complete. Saved entries:", Object.keys(results).length);
  ...
  console.log("favicon_whitelist_setup loaded. Call seedWhitelist([{domain, label}, ...]) to run.");
})();
