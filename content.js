...
if (window.__secureSightInjected) {
  ...
  (async function runChecks() {
    const results = {
      ...
      externalScripts: [],
      resourceCounts: { total: 0, external: 0, images: 0, scripts: 0, links: 0 },
    ...
    try {
      ...
      if ((url.match(/\./g) || []).length > 3) results.suspiciousSubdomainCount = true;
      ...
      if (mainWord && !title.includes(mainWord)) results.titleMatchesDomain = false;
      ...
      imgs.forEach(i => {
        try {
          ...
          if (srcHostname && srcHostname !== hostname) results.resourceCounts.external++;
        } catch (e) {}
      });
      ...
      links.forEach(l => {
        ...
        try {
          ...
          if (hrefHostname && hrefHostname !== hostname) results.resourceCounts.external++;
        } catch (e) {}
      });

      // Total resource count
      results.resourceCounts.total = results.resourceCounts.images + results.resourceCounts.scripts + results.resourceCounts.links;

      // Compute >50% external flag (store as a helper field)
      results.resourceAnalysis = {
        externalPercent: results.resourceCounts.total ? (results.resourceCounts.external / results.resourceCounts.total) * 100 : 0,
        majorityExternal: results.resourceCounts.total ? ((results.resourceCounts.external / results.resourceCounts.total) > 0.5) : false
      };
      ...
      if (iconEl && iconEl.href) faviconUrl = new URL(iconEl.href, window.location.origin).href;
      else if (appleTouch && appleTouch.href) faviconUrl = new URL(appleTouch.href, window.location.origin).href;
      else {
      ...
      const stored = await new Promise((resolve) => chrome.storage.local.get(['faviconHashWhitelist'], resolve));
    ...
    try {
      await new Promise((resolve) => chrome.storage.local.set({ lastScan: results }, resolve));
    } catch (err) {
