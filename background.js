...
async function fetchWhois(domain) {
  ...
  try {
    ...
    const createdDate = data.WhoisRecord && (data.WhoisRecord.createdDate || data.WhoisRecord.registryData && data.WhoisRecord.registryData.createdDate);
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "whois_lookup") {
    fetchWhois(msg.domain).then(date => sendResponse({ createdDate: date })).catch(() => sendResponse({ createdDate: null }));
    return true; // Keep channel open for async response
  }

  if (msg.type === "threat_lookup") {
    threatIntelLookup(msg.domain).then(result => sendResponse(result)).catch(() => sendResponse({ flagged: null }));
    return true;
  }
